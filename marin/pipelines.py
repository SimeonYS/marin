...
class MarinPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('marin.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `marin`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM marin WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO marin (date, title, link, content)"
